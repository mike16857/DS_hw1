...
public:
    ...
    int Compare(String y);         // compare two strings of letters of alphabet.
            ...
            //where xi, yj are letters, then the Compare member function will decide whether x<y, x=y, or x>y, 
...
int String::Compare(String y)
{
    ...
    // at least one of the string reaches the end
    if (i == Length() && j == y.Length()) return 0; // both strings reach the end
    else if (i == Length()) return -1;              // "this" string reaches the end
...
int main()
{
    ...
    // method Substr test
    int i, j;
    cout << "Input 'start' and 'length' for substring s1 (separate the two numbers by space): ";
    cin >> i >> j;
    String s4 = s1.Substr(i, j);
    cout << "s1.Substr(" << i << ", " << j << ") = " << s4 << "with length = " << s4.Length() << endl;
    cout << endl;

    // method Find, FastFind
    String pat(NULL, 0);
    cout << "Input a pattern to find in s1: " << endl;
    cin >> pat;
    ...
    // method Delete test
    int start, length;
    cout << "Which part do you want to delete in string s1 (input 'start' and 'length', separate the two numbers by space): ";
    cin >> start >> length;
    String s5 = s1.Delete(start, length);
    cout << "s1.Delete(" << start << ", " << length << ") = " << s5 << "with length = " << s5.Length() << endl;
    cout << endl;

    // method CharDelete test
    char c;
    cout << "Input a character to delete in string s1: ";
    cin >> c;
